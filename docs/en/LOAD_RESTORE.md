...
**`niri-session --load-last`** loads from the file set in config as **`[session].graceful_shutdown_name`** (by default file **`last`** in the [session directory](CONFIG.md#session-directory)); it is **`--load`** with a known path (the same snapshot **`--graceful-shutdown`** writes). Timings, **`[[launch]]`**, and the flag table below apply the same as for **`--load`**.

---

`niri-session --load [file]` (no argument — **`session.json`** in the [session directory](CONFIG.md#session-directory)):
